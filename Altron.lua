...
--------------------------------------------------------------------------------
-- АНТИ-ЧИТ БАЙПАСС И ОПТИМИЗИРОВАННЫЕ ХУКИ
--------------------------------------------------------------------------------
pcall(function()
...
-- Оптимизированный хук __namecall (работает мгновенно, не сажает FPS)
local OldNamecall
OldNamecall = hookmetamethod(game, "__namecall", function(self, ...)
    local method = getnamecallmethod()
    
    if not checkcaller() then
        -- Байпасс GetState для Флая (Говорим античиту, что мы бежим по земле)
...
local Window = Library:CreateWindow({
    Title = 'Neverlor | FPS Fix Edition', 
    Center = true, 
    AutoShow = true, 
    TabPadding = 8, 
    MenuFadeTime = 0.2
...
--------------------------------------------------------------------------------
-- ОПТИМИЗИРОВАННЫЕ МОДЫ ОРУЖИЯ (БОЛЬШЕ НЕ ЛАГАЕТ)
--------------------------------------------------------------------------------
...
-- Функция выполняется только 1 раз при взятии оружия (избавились от лагучего цикла getgc)
...
-- Хук на появление оружия вместо спама getgc каждые 1.5 секунды
local function SetupCharacterHooks(char)
    char.ChildAdded:Connect(function(c) 
        if c:IsA("Tool") then 
            task.wait(0.1) -- Ждем пока скрипты оружия прогрузятся
...
--------------------------------------------------------------------------------
-- ГЛАВНЫЕ ЦИКЛЫ ОБНОВЛЕНИЯ (Heartbeat + RenderStepped)
--------------------------------------------------------------------------------
local lastTargetPlr = nil
local lastInvCache = ""

-- Физический цикл (Heartbeat) - Плавный CFrame флай без лагов
table.insert(Cache.Connections, RunService.Heartbeat:Connect(function(deltaTime)
    ...
    if Toggles.FlyEnabled and Toggles.FlyEnabled.Value and RootPart and Humanoid then
        ...
        -- Полностью замораживаем физику, чтобы античит не видел скорость, без использования лагучих хуков
