--[[
    LebroTools - Ultimate Premium Edition (V3.2 - UI Polish & Intro)
    Разработчик: LebroTools Team
    Telegram: https://t.me/LebroToolsRost
    
    Модификация: 
    - Добавлено Intro (приветствие) с таймером 5 сек и авто-копированием ссылки.
    - Добавлены черные подложки под текст ESP (Name, Health).
    - Добавлен Watermark (FPS, Ник) и RGB линия (Chroma effect) для меню.
]]
...
print("Инициализация LebroTools Ultimate V3.2...")
...
EspGui.Name = "LebroToolsGUI_V3_2"
...
IntroTitle.Text = "🚀 LEBROTOOLS V3.2"
...
Watermark.Visible = false -- Включится после интро
...
btnUp.BackgroundColor3 = Theme.MainBg; btnUp.Text = "ВВЕРХ"
btnUp.TextColor3 = Theme.Text; btnUp.Font = Enum.Font.GothamBold; btnUp.TextSize = 12
Instance.new("UICorner", btnUp).CornerRadius = UDim.new(0, 10); Instance.new("UIStroke", btnUp).Color = Theme.Accent

local btnDown = Instance.new("TextButton", MobileNoclipUI)
btnDown.Size = UDim2.new(1, 0, 0, 70); btnDown.Position = UDim2.new(0, 0, 1, -70)
btnDown.BackgroundColor3 = Theme.MainBg; btnDown.Text = "ВНИЗ"
btnDown.TextColor3 = Theme.Text; btnDown.Font = Enum.Font.GothamBold; btnDown.TextSize = 12
Instance.new("UICorner", btnDown).CornerRadius = UDim.new(0, 10); Instance.new("UIStroke", btnDown).Color = Theme.Accent

btnUp.InputBegan:Connect(function(i) if i.UserInputType == Enum.UserInputType.Touch or i.UserInputType == Enum.UserInputType.MouseButton1 then Freecam.IsUp = true; Tween(btnUp, {BackgroundColor3 = Theme.Accent}, 0.1) end end)
btnUp.InputEnded:Connect(function(i) if i.UserInputType == Enum.UserInputType.Touch or i.UserInputType == Enum.UserInputType.MouseButton1 then Freecam.IsUp = false; Tween(btnUp, {BackgroundColor3 = Theme.MainBg}, 0.1) end end)
btnDown.InputBegan:Connect(function(i) if i.UserInputType == Enum.UserInputType.Touch or i.UserInputType == Enum.UserInputType.MouseButton1 then Freecam.IsDown = true; Tween(btnDown, {BackgroundColor3 = Theme.Accent}, 0.1) end end)
btnDown.InputEnded:Connect(function(i) if i.UserInputType == Enum.UserInputType.Touch or i.UserInputType == Enum.UserInputType.MouseButton1 then Freecam.IsDown = false; Tween(btnDown, {BackgroundColor3 = Theme.MainBg}, 0.1) end end)
...
InvFrame.Size = UDim2.new(0, 280, 0, 360); InvFrame.Position = UDim2.new(1, -300, 0.5, -180); InvFrame.BackgroundColor3 = Theme.MainBg; InvFrame.Visible = false
Instance.new("UICorner", InvFrame).CornerRadius = UDim.new(0, 8)
local InvStroke = Instance.new("UIStroke", InvFrame); InvStroke.Color = Theme.Border

local InvTitle = Instance.new("TextLabel", InvFrame)
InvTitle.Size = UDim2.new(1, 0, 0, 35); InvTitle.Text = "🎒 ИНВЕНТАРЬ ЦЕЛИ"; InvTitle.TextColor3 = Theme.Text; InvTitle.Font = Enum.Font.GothamBold; InvTitle.TextSize = 13; InvTitle.BackgroundColor3 = Theme.TopBar
...
InvList.Size = UDim2.new(1, -10, 1, -45); InvList.Position = UDim2.new(0, 5, 0, 40); InvList.BackgroundTransparency = 1; InvList.ScrollBarThickness = 2; InvList.ScrollBarImageColor3 = Theme.Accent
local InvLayout = Instance.new("UIGridLayout", InvList)
InvLayout.CellSize = UDim2.new(0, 62, 0, 62); InvLayout.CellPadding = UDim2.new(0, 6, 0, 6)
...
local function UpdateInventory(player)
    ...
    local function parseItem(tool, isUsing)
        ...
        else items[key] = {Name = name, Tex = id, Count = 1, Using = isUsing} end
    end
    ...
    for _, data in pairs(items) do
        ...
        Instance.new("UICorner", Slot).CornerRadius = UDim.new(0, 6); Instance.new("UIStroke", Slot).Color = Theme.Border
        
        if data.Tex then
            local Img = Instance.new("ImageLabel", Slot)
            Img.Size = UDim2.new(1, -10, 1, -10); Img.Position = UDim2.new(0, 5, 0, 5); Img.BackgroundTransparency = 1; Img.ScaleType = Enum.ScaleType.Fit; Img.Image = data.Tex
        else
            local Txt = Instance.new("TextLabel", Slot)
            Txt.Size = UDim2.new(1, -4, 1, -4); Txt.Position = UDim2.new(0, 2, 0, 2); Txt.BackgroundTransparency = 1; Txt.Text = data.Name; Txt.TextColor3 = Theme.TextDark; Txt.Font = Enum.Font.Gotham; Txt.TextSize = 10; Txt.TextWrapped = true
        end
        if data.Count > 1 then
            local Cnt = Instance.new("TextLabel", Slot)
            Cnt.Size = UDim2.new(0, 20, 0, 16); Cnt.Position = UDim2.new(1, -22, 1, -18); Cnt.BackgroundColor3 = Theme.Accent; Cnt.Text = "x"..data.Count; Cnt.TextColor3 = Color3.fromRGB(255,255,255); Cnt.Font = Enum.Font.GothamBold; Cnt.TextSize = 10
            Instance.new("UICorner", Cnt).CornerRadius = UDim.new(0, 4)
        end
        if data.Using then
            local Usg = Instance.new("Frame", Slot)
            Usg.Size = UDim2.new(1, 0, 0, 4); Usg.Position = UDim2.new(0, 0, 1, -4); Usg.BackgroundColor3 = Theme.Accent; Usg.BorderSizePixel = 0
...
Main.Visible = false -- Изначально скрыто до интро
...
-- RGB Chroma эффект для TopBarLine (Фишка "от себя")
task.spawn(function()
    local hue = 0
    while task.wait() do
        hue = hue + 0.005
...
ToggleBtn.Size = UDim2.new(0, 50, 0, 50); ToggleBtn.Position = UDim2.new(0, 15, 0, 50) -- Смещено чуть ниже ватермарки
...
ToggleBtn.Visible = false -- Скрыто до конца интро
...
AddSlider(secAimMain, "Скорость/Плавность аима", 1, 100, 20, "AimSmooth")
...
AddCheckbox(secPlrMove, "Visual Noclip (Ghost Mode / TP back)", "Noclip", function(val)
    ...
    if val then
        if UserInputService.TouchEnabled then MobileNoclipUI.Visible = true end 
        if hrp and hum then
            Freecam.SavedCFrame = hrp.CFrame
            hrp.Anchored = true 
            
            Freecam.CamPart = Instance.new("Part")
            Freecam.CamPart.Size = Vector3.new(1, 1, 1); Freecam.CamPart.Transparency = 1; Freecam.CamPart.CanCollide = false
            Freecam.CamPart.Anchored = true; Freecam.CamPart.CFrame = hrp.CFrame; Freecam.CamPart.Parent = Workspace
        ...
        if hrp then if Freecam.SavedCFrame then hrp.CFrame = Freecam.SavedCFrame end; hrp.Anchored = false end
            ...
            hum.HealthChanged:Connect(function(newHp)
                if newHp < oldHp and Toggles.HitSound then
                    if Cache.SilentTarget and Cache.SilentTarget.Parent == char then
                        local snd = Instance.new("Sound", SoundService); snd.SoundId = "rbxassetid://135478009117226"; snd.Volume = 3; snd:Play()
...
local function UpdateEnv()
    ...
    if Toggles.Fullbright then 
        Lighting.Ambient = Color3.fromRGB(255,255,255); Lighting.OutdoorAmbient = Color3.fromRGB(255,255,255); Lighting.GlobalShadows = false 
    else 
        Lighting.Ambient = Color3.fromRGB(127,127,127); Lighting.OutdoorAmbient = Color3.fromRGB(127,127,127); Lighting.GlobalShadows = true 
    end
    ...
    if Toggles.NoLeaves then 
        ...
        if tr then for _, i in ipairs(tr:GetDescendants()) do 
            if i:IsA("BasePart") and (i.Name:lower():find("leaf") or i.Name:lower():find("leavestop")) then i.Transparency = 1; i.CanCollide = false end 
        end end 
    end
end
...
task.spawn(function()
    while task.wait(0.2) do
        local hum = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Humanoid")
        if hum then
            if hum.WalkSpeed ~= Toggles.WalkSpeed then hum.WalkSpeed = Toggles.WalkSpeed end
            if Toggles.JumpPower ~= 50 then hum.JumpPower = Toggles.JumpPower; hum.UseJumpPower = true end
        end
    end
end)
                    ...
                    if rawget(v, "getfireDirection") and not Cache.ToolModHooked then
                        ...
                        v.getfireDirection = function(self, origin, raycast)
                            ...
                            if Toggles.SilentAim and Cache.SilentTarget then 
                                if not Toggles.OnlyOnHit or (Toggles.OnlyOnHit and isShooting) then return (Cache.SilentTarget.Position - origin).Unit end
                            end
...
----------------------------------------------------------------
-- [ ESP И ИНТЕЛЛЕКТУАЛЬНЫЙ РЕНДЕР (С ПОДЛОЖКАМИ!) ] --
----------------------------------------------------------------
local function CreateVisualsStruct()
    local obj = {
        Box = Drawing.new("Square"), BoxOut = Drawing.new("Square"),
        Name = Drawing.new("Text"), NameBg = Drawing.new("Square"), -- ДОБАВЛЕНА ПОДЛОЖКА ДЛЯ НИКА
        Dist = Drawing.new("Text"), 
        Health = Drawing.new("Text"), HealthBg = Drawing.new("Square"), -- ДОБАВЛЕНА ПОДЛОЖКА ДЛЯ HP
    ...
    obj.NameBg.Filled = true; obj.NameBg.Color = Color3.new(0,0,0); obj.NameBg.Transparency = 0.6; obj.NameBg.Thickness = 0
    ...
    obj.HealthBg.Filled = true; obj.HealthBg.Color = Color3.new(0,0,0); obj.HealthBg.Transparency = 0.6; obj.HealthBg.Thickness = 0
...
local function DrawSkeleton(char, espTable)
    ...
    for i, pair in ipairs(pairsToDraw) do
        ...
        if p1 and p2 then
            ...
            if on1 and on2 then espTable.Skeleton[i].Visible = true; espTable.Skeleton[i].From = Vector2.new(pos1.X, pos1.Y); espTable.Skeleton[i].To = Vector2.new(pos2.X, pos2.Y) end
        end
    end
end
...
RunService.RenderStepped:Connect(function()
    ...
    local function ProcessESP(entity, esp, isBot)
                ...
                if (isBot and Toggles.BotEsp or not isBot and Toggles.PlayerEsp) and dist <= Toggles.EspMaxDist then
                    ...
                    if passVis then
                        ...
                        if Toggles.EspName then
                            ...
                            esp.Name.Size = Toggles.FontSize
                            esp.Name.Position = Vector2.new(pos.X, pos.Y - h/2 - Toggles.FontSize - 5)
                            if isBot then esp.Name.Color = Color3.fromRGB(155, 89, 182) end
                            
                            local nWidth = esp.Name.TextBounds.X
                            esp.NameBg.Size = Vector2.new(nWidth + 6, Toggles.FontSize + 4)
                            esp.NameBg.Position = Vector2.new(esp.Name.Position.X - (nWidth/2) - 3, esp.Name.Position.Y - 2)
                            esp.NameBg.Visible = true
                        end

                        local bottom = pos.Y + h/2
                        
                        -- ESP Health с подложкой
                        esp.Health.Visible = Toggles.EspHealth
                        esp.HealthBg.Visible = false
                        if Toggles.EspHealth then 
                            esp.Health.Text = math.floor(entity.Humanoid.Health).." HP"
                            esp.Health.Size = Toggles.FontSize - 2
                            esp.Health.Position = Vector2.new(pos.X, bottom)
                            esp.Health.Color = Color3.fromHSV(math.clamp(entity.Humanoid.Health / entity.Humanoid.MaxHealth, 0, 1) * 0.3, 1, 1)
                            
                            local hpWidth = esp.Health.TextBounds.X
                            esp.HealthBg.Size = Vector2.new(hpWidth + 6, (Toggles.FontSize - 2) + 4)
                            esp.HealthBg.Position = Vector2.new(esp.Health.Position.X - (hpWidth/2) - 3, esp.Health.Position.Y - 2)
        ...
        if not isVisible then 
            esp.Box.Visible = false; esp.BoxOut.Visible = false; esp.Name.Visible = false; esp.Dist.Visible = false; esp.NameBg.Visible = false; esp.HealthBg.Visible = false
    ...
    if target then 
        ...
        if Toggles.SmoothSpeed < 20 then UI_Elements.TargetDot.Position = UI_Elements.TargetDot.Position:Lerp(endPos, Toggles.SmoothSpeed / 30) else UI_Elements.TargetDot.Position = endPos end
        ...
        if Toggles.TriggerBot and not isShooting then
            local mTarget = Mouse.Target; if mTarget and mTarget:IsDescendantOf(target.Parent) then if mouse1click then mouse1click() end end
        end
    end
end)
...
task.spawn(function()
    while task.wait(1) do
        if Toggles.InventoryViewer and Cache.SilentTarget then UpdateInventory(Players:GetPlayerFromCharacter(Cache.SilentTarget.Parent)); InvFrame.Visible = true
...
task.spawn(function()
    ...
    -- Прячем интро, показываем основной UI
    IntroFrame:Destroy()
    Main.Visible = true
    ToggleBtn.Visible = true
    Watermark.Visible = true
    print("LebroTools V3.2 успешно запущен!")
end)
