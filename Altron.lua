...
--------------------------------------------------------------------------------
-- АНТИ-ЧИТ БАЙПАСС И ХУКИ (МАКСИМАЛЬНЫЙ СТЕЛС ДЛЯ ФЛАЯ И АИМА)
--------------------------------------------------------------------------------
local Toggles = {} -- Пре-декларация для хуков
...
-- 1. Хук __index для скрытия скорости от локальных античитов
local OldIndex
OldIndex = hookmetamethod(game, "__index", function(self, key)
    if not checkcaller() and Toggles.FlyEnabled and Toggles.FlyEnabled.Value then
        -- Если античит пытается прочитать нашу скорость во время полета, отдаем ему "0"
        if self:IsA("BasePart") and (self.Name == "HumanoidRootPart" or self.Name == "Torso") then
            if key == "Velocity" or key == "AssemblyLinearVelocity" or key == "RotVelocity" or key == "AssemblyAngularVelocity" then
...
-- 2. Хук __namecall для Аима и подделки состояния (GetState)
local OldNamecall
OldNamecall = hookmetamethod(game, "__namecall", function(self, ...)
    local args = {...}
    local method = getnamecallmethod()
    
    -- Байпасс GetState для Флая (Говорим античиту, что мы бежим, а не падаем/летим)
    if not checkcaller() and Toggles.FlyEnabled and Toggles.FlyEnabled.Value then
    ...
    -- Сайлент Аим хук
    if Toggles.SilentAim and Toggles.SilentAim.Value and Cache.CurrentSilentTarget and not checkcaller() then
...
local Window = Library:CreateWindow({
    Title = 'Neverlor | Stealth Edition', 
    Center = true, 
    AutoShow = true, 
    TabPadding = 8, 
    MenuFadeTime = 0.2
...
local MobileFlyGroup = Tabs.Combat:AddRightGroupbox('CFrame Stealth Fly')
...
Toggles = _G.LinoriaToggles or getgenv().Toggles -- Синхронизация для хуков
...
task.spawn(function() while task.wait(1.5 + math.random()) do if Toggles.SilentAim.Value then ProcessToolMods() end end end)
table.insert(Cache.Connections, LocalPlayer.CharacterAdded:Connect(function(char) char.ChildAdded:Connect(function(c) if c:IsA("Tool") then task.spawn(ProcessToolMods) end end) end))

--------------------------------------------------------------------------------
-- ГЛАВНЫЕ ЦИКЛЫ ОБНОВЛЕНИЯ (RenderStepped для UI, Heartbeat для Физики Флая)
--------------------------------------------------------------------------------
local lastTargetPlr = nil
local lastInvCache = ""

-- Физический цикл (Heartbeat) - Оптимально для CFrame флая
table.insert(Cache.Connections, RunService.Heartbeat:Connect(function(deltaTime)
    ...
    if Toggles.FlyEnabled and Toggles.FlyEnabled.Value and RootPart and Humanoid then
        ...
        -- Полностью замораживаем физику. Это предотвращает Rubberbanding античитом на сервере!
        RootPart.AssemblyLinearVelocity = Vector3.zero
        RootPart.AssemblyAngularVelocity = Vector3.zero
        Humanoid.PlatformStand = true
        
        -- Получаем направление движения на основе камеры
        local moveDir = Humanoid.MoveDirection
        local speed = Options.FlySpeed.Value
        
        -- Обработка мобильных кнопок и кнопок ПК (Space/Ctrl)
        local yMove = 0
        ...
        -- Плавное и безопасное перемещение через CFrame
        local velocityToApply = (moveDir * speed * deltaTime) + (Vector3.new(0, yMove, 0) * (speed * 0.8 * deltaTime))
        
        -- Расчет поворота лицом по направлению камеры
