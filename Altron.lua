...
local function UpdateEnv()
    ...
    pcall(function()
        if Workspace:FindFirstChild("Terrain") then 
            Workspace.Terrain.Decoration = not Toggles.NoGrass 
        end
    end)
    ...
    pcall(function()
        if Toggles.NoLeaves then 
            ...
            if tr then 
                for _, i in ipairs(tr:GetDescendants()) do 
                    if i:IsA("BasePart") and (string.find(string.lower(i.Name), "leaf") or string.find(string.lower(i.Name), "leavestop")) then 
                        i.Transparency = 1
                        i.CanCollide = false 
                    end 
                end 
...
local function RefWEsp()
    pcall(function()
        ...
        table.clear(Cache.WorldInstances)

        if Toggles.HempEsp then 
            local f = Workspace:FindFirstChild("Hemp")
            if f then for _, i in ipairs(f:GetChildren()) do AddChams(i, Color3.fromRGB(0, 255, 0), "Hemp") end end 
        end

        if Toggles.CrateEsp then 
            local f = Workspace:FindFirstChild("crates") or Workspace:FindFirstChild("Crates")
            if f then for _, i in ipairs(f:GetDescendants()) do 
                if i.Name == "MilitaryCrate" or i.Name == "Crate" or i.Name == "ToolBox" then 
                    AddChams(i, Color3.fromRGB(255, 165, 0), i.Name) 
                end 
            end end 
        end

        if Toggles.BpEsp then 
            local f = Workspace:FindFirstChild("DeathBackpacks")
            if f then for _, i in ipairs(f:GetChildren()) do AddChams(i, Color3.fromRGB(255, 0, 0), "Backpack") end end 
        end

        if Toggles.TcEsp then 
            local f = Workspace:FindFirstChild("Builds")
            if f then for _, i in ipairs(f:GetDescendants()) do 
                if i.Name == "ToolCupboardModel" then AddChams(i, Color3.fromRGB(0, 150, 255), "TC") end 
            end end 
        end
    end)
end
    ...
    while task.wait(3) do
        ...
        pcall(function()
            if Toggles.CustomSky and not Lighting:FindFirstChild("LebroSky") then
                for _, obj in pairs(Lighting:GetChildren()) do 
                    if obj:IsA("Atmosphere") or obj:IsA("Sky") then 
                        table.insert(Cache.OriginalLighting, obj)
                        obj.Parent = nil 
                    end 
                end
            ...
            elseif not Toggles.CustomSky and Lighting:FindFirstChild("LebroSky") then
                ...
                for _, obj in pairs(Cache.OriginalLighting) do 
                    obj.Parent = Lighting 
                end
                table.clear(Cache.OriginalLighting)
            end
        end)
    end
end)
        ...
        pcall(function()
            for _, v in pairs(getgc(true)) do
                if type(v) == "table" then
                    ...
                    if rawget(v, "BaseBulletVelocity") and Toggles.MaxVel then 
                        v.BaseBulletVelocity = 999999
                        v.Velocity = 999999 
                    end
                    if rawget(v, "BulletGravity") and Toggles.ZeroGrav then 
                        v.BulletGravity = 0
                        v.Gravity = 0 
                    end
...
Players.PlayerRemoving:Connect(function(p) 
    if Cache.PlayerVisuals[p] then 
        for _, v in pairs(Cache.PlayerVisuals[p]) do 
            if typeof(v)=="Instance" then v:Destroy() else v:Remove() end 
        end
        Cache.PlayerVisuals[p] = nil 
    end 
end)
...
print("LebroTools загружен успешно! Откройте меню кнопкой 'МЕНЮ' слева сверху.")
